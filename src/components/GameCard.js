...
const GameCard = ({ name, image, metacritic, releaseDate, id }) => {
  return (
    <Link to={`/game/${id}`}>
      <main className="rounded-lg w-full relative cursor-pointer overflow-hidden border-[1.5px] border-light hover:bg-lighter-dark transition-all duration-300 game-card max-md:rounded-xl">
        <img
          src={image}
          alt={name}
